...
/*TEMPAI 資料庫裡的ai ; TEMPcount 給使用者看的第幾筆 */
$TEMPAI = $_GET['id'];
$TEMPcount = $_GET['tc'];
$email = @$_GET['email'];
$name = @$_GET['name'];
...
$sql ="DELETE FROM registerclassroom WHERE AI=$TEMPAI";  //刪除資料

if (mysqli_query($link,$sql)){
	echo "第 ",$TEMPcount," 筆刪除成功";	
	echo '<meta http-equiv=REFRESH CONTENT=0;url=../ruler/classroommanage.php>';
}else 
	echo '刪除失敗'; //執行sql語法
echo '<meta http-equiv=REFRESH CONTENT=0;url=../ruler/classroommanage.php>';
...
<?php
   ...
   $mail->Subject = "=?UTF-8?B?".base64_encode("審核通過通知")."?=";    //設定郵件標題
	...
	$mail->Body = "$name 你選的 $rs[3] 到 $rs[4] 教室 $rs[5] 借用失敗 請重新選擇";
